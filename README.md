...
**현재: 5단계** — Kafka 이벤트 (실시간은 Redis·WS)
...
1. 회원가입 · 로그인 (JWT) — 완료
2. 채팅방 · 1:1 · 그룹 · 메시지 REST — 완료
3. WebSocket · Redis Pub/Sub — 완료
...
| 항목 | 값 |
|------|-----|
| URL | `ws://localhost:8081/ws/chat?token={accessToken}` |
| JOIN | `{"type":"JOIN","roomId":1}` |
| SEND | `{"type":"SEND","roomId":1,"content":"안녕","messageType":"TEXT"}` |

상세: [`docs/WEBSOCKET.md`](docs/WEBSOCKET.md)

### 5단계 — Kafka

- 토픽: `chat.message.sent`
- Redis 실시간 push 유지, Kafka는 이벤트 기록·확장용
- 상세: [`docs/KAFKA.md`](docs/KAFKA.md)

### Postman (테스트)

| 용도 | 파일 |
|------|------|
| REST | `postman/Chatflow-Server.postman_collection.json` |
| WebSocket | **`postman/ws-test.html`** 또는 [postman/README-WebSocket.md](postman/README-WebSocket.md) (Postman JSON Import는 GET으로만 열림) |
